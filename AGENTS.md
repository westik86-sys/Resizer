...
- The happy path is: select or drop video, probe it, choose a preset and output folder, transcode with visible progress, validate the temporary result, then publish the final copy and size comparison.
...
- The public MVP adds multi-file import, a sequential queue, three presets, bounded typed settings, safe output naming, progress/ETA, cancel/retry, results, notifications, and FFmpeg version/license disclosure.
...
- The Application coordinator owns the workflow and the single-source-of-truth `JobState`, including valid transitions and `probe -> preflight -> encode -> validate -> commit`.
...
- Represent encoding configuration with validated, typed models. Represent FFmpeg arguments only as `[String]`.
...
- Detect and cache the bundled build's actual capabilities before presenting encoder-dependent settings.
...
- Publish final output only after exit code handling, EOF from both pipes, and successful `ffprobe` validation of the temporary result.
...
- Keep security-scoped access alive through encode, validation, and final commit.
...
- Unit-test the state machine, probe/progress parsers, command builder, presets, naming, and output policy.
