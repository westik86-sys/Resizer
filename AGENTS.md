...
- Quick deterministically uses software libx264 CRF `22` with preset `medium` for every supported SDR source, at most 1920x1080 and 30 FPS. Sources at or below 8-bit produce compatible limited-range `yuv420p`; confirmed sources above 8-bit produce 10-bit H.264 while preserving supported source chroma subsampling as `yuv420p10le`, `yuv422p10le`, or `yuv444p10le`. When audio is kept, the selected mono stream uses AAC 69 kbit/s; stereo, multichannel, and unknown channel counts use AAC 128 kbit/s. Flexible exposes only bounded quality `0.30...0.90`, source/1080p/720p/480p resolution, source/60/30/24 FPS, and keep/remove audio while retaining the same bit-depth/chroma and audio policies; its quality maps through `CRF = 36 - floor(12 * qualityPercent / 100)`. `compactRetry` uses libx264 CRF `31`, at most 1280x720 and 24 FPS, and inherits the Quick keep/remove audio choice. No mode may upscale resolution or frame rate.
...
- The FFmpeg default is a reproducible GPL 2.0-or-later build with statically linked libx264, as recorded in ADR 0014 and amended by ADR 0016.
- Keep `--enable-gpl`, the pinned libx264 source, and its `--bit-depth=all` / `--chroma-format=all` configuration explicit and reproducible. Never enable `--enable-version3`, `--enable-nonfree`, or `libx265` without a separate explicit decision.
