...
- Quick deterministically uses HEVC VideoToolbox Main10 quality `0.70` for confirmed >8-bit SDR sources and software libx264 CRF `24` with preset `medium` otherwise, at most 1920x1080 and 30 FPS. When audio is kept, the selected mono stream uses AAC 69 kbit/s; stereo, multichannel, and unknown channel counts use AAC 128 kbit/s. Flexible exposes only bounded quality `0.30...0.90`, source/1080p/720p/480p resolution, source/60/30/24 FPS, and keep/remove audio while retaining the same source-derived codec and audio policies; its H.264 quality maps through `CRF = 36 - floor(12 * qualityPercent / 100)`. `compactRetry` uses HEVC quality `0.60` or libx264 CRF `31`, at most 1280x720 and 24 FPS, and inherits the Quick keep/remove audio choice. No mode may upscale resolution or frame rate.
