...
| Видеовыход MVP | H.264 8-bit через software libx264 для обычных источников; HEVC Main10 через VideoToolbox для подтверждённых 10-bit SDR |
| Аудио | AAC |
...
- для остальных поддерживаемых источников — software H.264 через `libx264`,
  limited-range `yuv420p`, CRF `24` и preset `medium`;
- при вынужденном уменьшении глубины используется error-diffusion dithering;
- HEVC-качество отображается в поддерживаемый VideoToolbox `global_quality`;
  H.264 Quick использует фиксированный CRF 24;
...
поддерживаемые источники создают совместимый H.264 через libx264. Для H.264
нормализованное качество отображается по формуле CompressO
...
ограничениями, а mapping на libx264 или VideoToolbox остаётся внутренним
типизированным контрактом.
...
- `--enable-gpl`, pinned static `libx264` core 165/r3223;
- без `--enable-version3`, `--enable-nonfree` и `libx265`;
- H.264 output через libx264; HEVC Main10 через `hevc_videotoolbox`;
...
- без --enable-nonfree;
- без libx265;
- с native H.264/HEVC decoder/parser, `libx264`,
...
MVP:
- MP4;
- libx264 для обычных 8-bit источников;
- hevc_videotoolbox Main10 для подтверждённых 10-bit SDR источников;
- AAC;
- native selector Quick/Flexible;
- quick recipe: H.264 CRF 24/preset medium либо HEVC Main10 quality 0.70, максимум
