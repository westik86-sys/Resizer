...
- Один режим «Автоматически» на первом уровне; выбор пресета и технические параметры не входят в основной сценарий.
- Автоматическая политика должна быть типизированной, детерминированной, тестируемой и объяснимой, а не набором скрытых произвольных FFmpeg-флагов.
...
- Один пользовательский режим «Автоматически» без выбора пресета и ручных параметров кодирования.
- Детерминированный выбор recipe из результатов `ffprobe` по контракту автоматического режима ниже.
...
- Результат:
  ...
  - «Открыть»;
  - «Показать в Finder»;
  - вторичное действие «Сжать ещё сильнее».
- Системное уведомление, если приложение неактивно.
- Экран лицензий и точная версия FFmpeg.

### Контракт автоматического режима MVP

Этот продуктовый контракт заменяет прежний обязательный выбор из трёх
пресетов. Он не меняет принятые ограничения кодека, лицензии, sandbox,
неизменяемости входа, временного файла, валидации и безопасной публикации.
Перед изменением production-кода нужно отдельным этапом обновить ADR,
архитектурную документацию, `AGENTS.md` и тестовые контракты, которые всё ещё
описывают три пресета.

После успешного `ffprobe` приложение строит recipe автоматически. В публичном
MVP это быстрые детерминированные правила по метаданным, а не классификация
содержимого и не пробное кодирование:

- MP4, H.264 через `h264_videotoolbox`, limited-range `yuv420p` и `faststart`;
- качество видео `0,65`, отображаемое в поддерживаемый VideoToolbox
  `global_quality`;
...
- AAC 128 кбит/с при наличии выбранной аудиодорожки, иначе без аудио;
...
«Сжать ещё сильнее» — осознанное вторичное действие после первого результата.
Оно всегда повторно кодирует неизменяемый оригинал, а не уже сжатую копию, и
использует фиксированную compact-политику: максимум 1280×720, максимум 24 FPS,
качество `0,45`, AAC 96 кбит/с. Эта политика не показывается как пресет перед
первым запуском.
...
job и завершает сценарий нейтральным результатом «Видео уже достаточно
компактно». Это не ошибка кодирования. Точный размер до завершения не
обещается.

### Не включать в MVP

- Гарантированный целевой размер.
- Ручной выбор разрешения, FPS, качества, аудиобитрейта или метаданных.
- Пользовательские пресеты и произвольные комбинации настроек.
...
- В центре перед запуском — краткое read-only резюме автоматического результата
  и папка назначения.
...
- Метка «Автоматически» без selector.
- Краткое read-only резюме, например: `MP4 · H.264 · до 1080p · до 30 FPS · AAC`.
- Папка результата.
- Одна primary-кнопка «Сжать».

Не показывать `CRF`, `global_quality`, битрейт видео или другие параметры
кодировщика в основном интерфейсе: VideoToolbox и программные кодировщики
используют разные модели управления качеством, а автоматическая политика
остаётся внутренним продуктовым контрактом.
...
не был опубликован. Secondary-действие «Сжать ещё сильнее» создаёт новый запуск
compact-политики от оригинала.
...
- `CompressionMode` (`automatic`, `compactRetry`);
- `RateControl`;
- `ScalePolicy`;
- `FrameRatePolicy`;
- `MetadataPolicy`;
...
- Автоматическая политика никогда не увеличивает разрешение или FPS и всегда
  повторяемо строит один и тот же recipe для одинакового `MediaInfo`.
...
| 6. Automatic recipe | Типизированная auto/compact policy и детерминированный argument builder | Golden tests для обеих политик и всех ветвей MediaInfo |
...
CompressionRecipe, AutomaticCompressionPolicy, CompressionMode, RateControl,
...
### Промпт 6 — automatic recipe и command builder

```text
Цель: реализовать типизированную автоматическую политику сжатия,
...
- h264_videotoolbox;
- AAC;
- один видимый режим Automatic;
- automatic recipe: quality 0.65, максимум 1080p/30 FPS, AAC 128 кбит/с;
- compact retry: quality 0.45, максимум 720p/24 FPS, AAC 96 кбит/с;
...
Compact retry всегда строится от оригинала, не от уже сжатой копии.

Не добавляй selector пресетов, ручные параметры, arbitrary FFmpeg flags,
пробное кодирование, HEVC-выход или target-size mode.

Добавь golden tests для automatic и compact retry, границ 1080p/30 FPS,
Unicode-путей, video без audio, resize и output collision.
```
...
- карточку метаданных;
- метку «Автоматически» и read-only резюме результата;
- выбор output directory;
- одну primary-кнопку Start, а также Cancel, Retry и Reveal in Finder;
- progress, ETA и speed;
- вторичное действие «Сжать ещё сильнее» после успешного automatic result;
...
Не показывай выбор пресета, CRF или ручные параметры кодирования.
