...
- Downloads как выходная папка по умолчанию без обязательного диалога;
  пользователь может выбрать другую папку для текущей сессии.
...
- Результат:
  ...
  - «Открыть»;
  - «Показать в Finder».
- После завершения текущей запущенной очереди Finder открывается один раз и
  выделяет все успешно опубликованные результаты. Для `NoBenefit`, ошибки или
  отмены Finder не открывается; автоматическое действие можно отключить в
  настройках.
...
- Quick использует фиксированный CRF 22; получающиеся профили High 10,
  ...
  4:2:0 H.264;
- сохраняет исходное разрешение без уменьшения; для совместимого H.264 с
  4:2:0 нечётные стороны нормализуются вниз до ближайшего чётного значения;
...
- В гибком режиме — native Slider качества, Picker разрешения, Picker FPS и
  toggle звука.
- Краткое резюме, например: `MP4 · H.264 · исходное разрешение · до 30 FPS · AAC`
  либо `MP4 · H.264 10-bit 4:4:4 · исходное разрешение · 24 FPS (как в
  оригинале) · AAC`.
...
- Папка результата, по умолчанию Downloads, и вторичное действие «Изменить…».
...
- Автоматически показывать успешно сохранённые результаты в Finder после
  завершения текущей очереди (включено по умолчанию).
...
- Получать системную Downloads через `FileManager` и `.downloadsDirectory`,
  никогда не собирать путь из `~` или имени папки вручную; в composition root
  разрешать служебную sandbox-symlink только для этого доверенного
  системного URL до общей проверки, которая по-прежнему запрещает symlink.
- Объяснять системный запрос доступа через локализованный
...
- quick recipe: H.264 CRF 22/preset medium, исходное разрешение и максимум
  30 FPS, AAC 69 кбит/с для выбранной mono-дорожки и 128 кбит/с для остальных
  или неизвестных channel count, либо удаление audio;
