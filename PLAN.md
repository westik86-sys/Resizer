...
| Видеовыход MVP | H.264 8-bit для обычных источников; HEVC Main10 через VideoToolbox для подтверждённых 10-bit SDR |
| Аудио | AAC |
...
- нормализованное качество отображается в поддерживаемый VideoToolbox
  `global_quality`;
...
- AAC 128 кбит/с при включённом «Сохранить звук» и наличии выбранной
  аудиодорожки, иначе без аудио;
...
- сохранить AAC 128 кбит/с либо полностью убрать звуковую дорожку.

Гибкий режим использует ту же детерминированную политику codec/bit depth, что
и быстрый: подтверждённый 10-bit SDR сохраняется как HEVC Main10, остальные
поддерживаемые источники создают совместимый H.264. Ручного выбора кодека нет.
...
«Сжать ещё сильнее» — осознанное вторичное действие после результата быстрого
режима.
...
качество `0,45` для H.264 либо `0,60` для HEVC Main10, AAC 96 кбит/с либо без
аудио, если оно было выключено в быстром режиме. Эта политика не показывается
как пресет перед первым запуском.
...
- Краткое резюме, например: `MP4 · H.264 · до 1080p · до 30 FPS · AAC` либо
  `MP4 · HEVC 10-bit · до 1080p · 24 FPS (как в оригинале) · AAC`.
...
Не показывать `CRF`, `global_quality`, битрейт видео или произвольные параметры
кодировщика: пользователь управляет нормализованным качеством и продуктовыми
ограничениями, а их mapping на VideoToolbox остаётся внутренним контрактом.
...
- `CompressionMode` (`automatic`, `compactRetry`) как тип попытки;
...
- использовать `h264_videotoolbox` и `hevc_videotoolbox` без `libx264` или
  `libx265`;
...
| 6. Compression recipes | Типизированные Quick/Flexible/compact policy и детерминированный argument builder | Golden tests для политик, audio remove и всех ветвей MediaInfo |
...
- без --enable-nonfree;
- без libx264/libx265;
- с native H.264/HEVC decoder/parser, `h264_videotoolbox`,
  `hevc_videotoolbox` и native AAC.
...
```text
Цель: реализовать типизированные Quick/Flexible/compact политики сжатия,
CompressionRecipe и чистый FFmpegCommandBuilder.

MVP:
- MP4;
- h264_videotoolbox для обычных 8-bit источников;
...
- compact retry: H.264 quality 0.45 либо HEVC Main10 quality 0.60, максимум
  720p/24 FPS, AAC 96 кбит/с либо без аудио — в соответствии с выбором Quick;
...
Добавь golden tests для Quick/Flexible/compact retry, keep/remove audio,
...
- вторичное действие «Сжать ещё сильнее» после результата Quick;
